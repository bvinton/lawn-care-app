...
var MAINTENANCE_TASK_NAMES = /* @__PURE__ */ new Set([MOW_TASK_NAME, WATER_TASK_NAME]);
...
function isSyntheticMaintenanceComplete(task) {
  return task.status === "completed" && typeof task.reason === "string" && /dormancy|winter|rain|not actively growing|nature providing/i.test(task.reason);
}
function buildMaintenanceSyncRow(task, maintenance, existingRows, todayStr) {
  const localLast = task.title === MOW_TASK_NAME ? maintenance.lastMowedDate : task.title === WATER_TASK_NAME ? maintenance.lastWateredDate : null;
...
async function runLawnCloudSync() {
  ...
  const maintenanceRows = inboundRows.filter(
    (row) => ["Mow lawn", "Water lawn"].includes(row.task_name)
  );
